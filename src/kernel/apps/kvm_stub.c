...
static int emulate_instruction(vm_t *vm) {
    ...
    switch (opcode) {
        ...
        // TEST accumulator immediate: A8 (AL, imm8), A9 (AX, imm16)
        case 0xA8: { uint8_t imm = fetch8(vm); uint8_t val = vm->cpu.ax & 0xFF; update_logic_flags(vm, val & imm, 8); break; }
        case 0xA9: { uint16_t imm = fetch16(vm); uint16_t val = vm->cpu.ax; update_logic_flags(vm, val & imm, 16); break; }

        // TEST r/m, imm8/imm16: F6 /0 (8-bit), F7 /0 (16-bit)
        case 0xF6: {
            decode_modrm(vm, &m);
            // only valid when /0 (reg==0)
            uint8_t imm8 = fetch8(vm);
            uint8_t dst8 = rm_read(vm, &m, 0) & 0xFF;
            update_logic_flags(vm, dst8 & imm8, 8);
            break;
        }
        case 0xF7: {
            decode_modrm(vm, &m);
            uint16_t imm16 = fetch16(vm);
            uint16_t dst16 = rm_read(vm, &m, 1);
            update_logic_flags(vm, dst16 & imm16, 16);
            break;
        }
