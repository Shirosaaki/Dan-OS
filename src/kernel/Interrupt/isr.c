...
void divide_by_zero_handler(interrupt_frame_t* frame) {
    terminal_writestring("FATAL: Division by zero at RIP: ");
...
void page_fault_handler(interrupt_frame_t* frame) {
    ...
    terminal_writestring("Error code: ");
    if (frame->err_code & 0x1) terminal_writestring("Present ");
    if (frame->err_code & 0x2) terminal_writestring("Write ");
    else terminal_writestring("Read ");
    if (frame->err_code & 0x4) terminal_writestring("User ");
    else terminal_writestring("Supervisor ");
    terminal_writestring("\n");
